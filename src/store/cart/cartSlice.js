...
//     state.cart.items.some(item => item.item.id === productId);
...
const cartSlice = createSlice({
    ...
    reducers: {
        ...
        setToCart: (state, action) => {
            ...
            if (existingItem) {
                existingItem.count += 1;
                existingItem.price = Number(existingItem.item.price) * existingItem.count;
            } else {
                const cleanItem = {
                    item: {
        ...
        incrementCount: (state, action) => {
            ...
            if (itemToUpdate) {
                itemToUpdate.count += 1;
                itemToUpdate.price = itemToUpdate.item.price * itemToUpdate.count;
                localStorage.setItem("readyForBuy", JSON.stringify(state.items));
            }
        },
        decrementCount: (state, action) => {
